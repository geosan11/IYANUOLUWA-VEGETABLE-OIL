...
| Server-backed persistence (Supabase/Postgres) instead of `localStorage` only | 🚧 — **wired and live** are auth (`auth.users`/`profiles`) plus every Settings-managed master table (`hubs`, `app_settings`, `suppliers`, `physical_tanks`, `pumps`, `products`, `product_varieties`), the logo in Storage, and the public branding RPC. **Wired and pending migration** is the transactional layer: `src/services/ledger.ts` pulls `customers`, stock intakes (`tanks`), `sales`, sale lines (`orders`) and tender legs (`sale_payments`) and `payments`, uploads rows that exist only on the device, and mirrors every later sale/payment/void through a durable outbox — but it probes for the `sales` table first and stays completely inert until migrations 0019–0022 are applied, so today it is still `localStorage` only in practice. **Still `localStorage`-only** (no sync code yet): `expenses`, `shifts`, `pump_readings`, `keg_returns`, `transfers`, `tank_dipstick_readings`, `customer_credits` — plus the pack-price matrix, whose `pack_prices` table exists (0020) but has no reader/writer in the app yet. |
