...
| Server-backed persistence (Supabase/Postgres) instead of `localStorage` only | 🚧 — **wired and live** are auth (`auth.users`/`profiles`) plus every Settings-managed master table (`hubs`, `app_settings`, `suppliers`, `physical_tanks`, `pumps`, `products`, `product_varieties`), the logo in Storage, and the public branding RPC. **Wired, and its migrations are now applied** (0018–0022 pushed to the live project on 25 Sep 2026) is the transactional layer: `src/services/ledger.ts` pulls `customers`, stock intakes (`tanks`), `sales`, sale lines (`orders`) and tender legs (`sale_payments`) and `payments`, uploads rows that exist only on the device, and mirrors every later sale/payment/void through a durable outbox. It still probes for the `sales` table before doing any of that, so an un-migrated database keeps the old `localStorage`-only behaviour — but the only thing left before sales actually reach Postgres is deploying this build (the live site is still the pre-ledger bundle). **Still `localStorage`-only** (no sync code yet): `expenses`, `shifts`, `pump_readings`, `keg_returns`, `transfers`, `tank_dipstick_readings`, `customer_credits` — plus the pack-price matrix, whose `pack_prices` table exists (0020) but has no reader/writer in the app yet. |
