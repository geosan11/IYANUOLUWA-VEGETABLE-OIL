...
export const Sidebar: React.FC<SidebarProps> = ({ currentTab, onTabChange }) => {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-72 bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800/80 h-screen select-none flex-shrink-0 z-30 transition-colors duration-200">
      {/* Brand Header */}
      <div className="p-6 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between">
        <div className="flex items-center gap-3.5">
          {settings.company_logo_url ? (
            <img
              ...
              alt="Logo"
              className="w-10 h-10 object-contain rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 p-1 shadow-sm"
            />
          ) : (
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[11px] font-semibold text-brand-600 dark:text-brand-400 uppercase tracking-wider flex items-center gap-1.5">
        ...
        {/* Quick theme toggle in brand header */}
        <button
          onClick={toggleTheme}
          ...
          return (
            ...
            >
              ...
              {item.badge !== null && item.badge > 0 && (
                <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-400 border border-rose-300 dark:border-rose-500/30">
                  {item.badge}
                </span>
              )}
            </button>
        ...
        {activeAlerts.totalAlertCount > 0 && (
          <div className="pt-4 px-1">
            <div
              ...
              className="cursor-pointer p-3 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 hover:bg-rose-100/80 dark:hover:bg-rose-950/60 transition-colors shadow-sm"
            >
      ...
      <div className="p-3 border-t border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-900/50">
        <div className="p-3 rounded-xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-brand-600 dark:text-brand-400">
