    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 bg-white text-slate-900" id="receipt-print-area">
          ...
          <div className="text-center border-b-2 border-dashed border-slate-300 pb-4 mb-4">
            {settings.company_logo_url ? (
              ...
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-brand-500 text-white font-extrabold text-xl mb-2 shadow-sm">
                IO
              </div>
            )}
            <h1 className="text-lg font-extrabold tracking-tight text-slate-950 uppercase">
              {settings.company_name}
            </h1>
            <p className="text-xs text-slate-600 mt-0.5">{settings.company_address}</p>
            <p className="text-xs text-slate-600 font-mono">Tel: {settings.company_phone}</p>
            
            <div className="mt-3 inline-block px-3 py-1 bg-slate-100 rounded-full border border-slate-300 text-xs font-bold font-mono uppercase tracking-wider text-slate-800">
              OFFICIAL RECEIPT
            </div>
          </div>

          {/* Metadata Grid */}
          <div className="grid grid-cols-2 gap-2 text-xs border-b border-slate-200 pb-3 mb-3 font-mono">
            <div>
            ...
            <div>
              ...
              <span className="font-bold text-slate-900">{receipt.customer.name}</span>
              <span className="text-[10px] text-slate-500 block uppercase">({receipt.customer.type})</span>
            </div>
            <div className="text-right">
              <span className="text-slate-500 block text-[10px] uppercase">Payment Method:</span>
              <span className="font-bold uppercase text-slate-900 px-1.5 py-0.5 bg-slate-100 rounded">
          ...
          {isOrder && order ? (
            <div className="border-b-2 border-dashed border-slate-300 pb-3 mb-3">
              ...
              <table className="w-full text-xs font-mono">
                <thead>
                  <tr className="text-slate-500 text-[10px] border-b border-slate-200 pb-1">
                ...
                <tbody className="divide-y divide-slate-100">
                  <tr>
                    <td className="py-2 text-left">
                      <div className="font-bold text-slate-900">{receipt.product?.name}</div>
              ...
              {order.note && (
                <div className="mt-2 text-[11px] text-slate-600 bg-slate-50 p-2 rounded border border-slate-200">
            ...
            <div className="border-b-2 border-dashed border-slate-300 pb-3 mb-3">
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 text-center my-2">
                <span className="text-xs text-emerald-800 block uppercase font-medium">
              ...
              {receipt.unappliedLeftover && receipt.unappliedLeftover > 0 ? (
                <div className="text-xs font-mono text-amber-800 bg-amber-50 p-2 rounded border border-amber-200 mt-2">
                  <span className="font-bold">Note:</span> ₦{receipt.unappliedLeftover.toLocaleString()} was not applied as all open credit invoices are fully settled.
                </div>
              ) : null}
            </div>
          )}
          ...
          {/* Balances & Totals */}
          <div className="space-y-1 text-xs font-mono border-b border-slate-200 pb-3 mb-3">
            {isOrder && order && (
              <>
          ...
          {/* Footer Sign-off */}
          <div className="text-center text-[11px] text-slate-500 pt-1 space-y-1">
        ...
        <div className="no-print p-4 bg-slate-900 border-t border-slate-800 flex items-center justify-end gap-3">
          ...
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-slate-950 font-bold text-xs shadow-lg shadow-brand-500/25 transition-all transform active:scale-95"
          >
