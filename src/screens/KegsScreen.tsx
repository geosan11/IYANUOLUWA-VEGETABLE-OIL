      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-2">
            ...
            >
              {isEditingTotalKegs ? <Check className="w-3.5 h-3.5 text-brand-600 dark:text-brand-400" /> : <Edit2 className="w-3.5 h-3.5" />}
            </button>
          </div>
          ...
          {isEditingTotalKegs ? (
            <div className="flex items-center gap-2 my-1">
              <input
                ...
                className="w-28 px-3 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-brand-500 text-base font-mono font-bold text-slate-900 dark:text-white focus:outline-none"
                autoFocus
              />
              <button
            ...
            <div className="text-2xl font-black font-mono text-slate-900 dark:text-slate-100">
              {settings.total_company_kegs} <span className="text-xs font-normal text-slate-500 dark:text-slate-400">total owned</span>
            </div>
          )}
        ...
        <div
          className={`p-5 rounded-2xl border flex flex-col justify-between transition-all shadow-sm ${
            kegInventory.isDepotStockCritical
              ? 'bg-rose-50 dark:bg-rose-950/40 border-rose-300 dark:border-rose-500/60 shadow-rose-500/10 animate-pulse'
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            <RotateCcw
              className={`w-4 h-4 ${
                kegInventory.isDepotStockCritical ? 'text-rose-600 dark:text-rose-400' : 'text-brand-600 dark:text-brand-400'
              }`}
            />
          </div>
          <div
          ...
          >
            {kegInventory.kegsAtDepot} <span className="text-xs font-normal text-slate-500 dark:text-slate-400">available</span>
          </div>
          <div
            className={`text-[10px] mt-2 font-semibold ${
              kegInventory.isDepotStockCritical ? 'text-rose-700 dark:text-rose-300' : 'text-slate-400'
            }`}
          >
            {kegInventory.isDepotStockCritical
                ...
                return (
                  <tr key={cust.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="py-3 px-3">
                      <div className="font-bold text-slate-900 dark:text-slate-200 text-sm font-sans">{cust.name}</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">{cust.phone}</div>
                    </td>
                    <td className="py-3 px-3">
                    ...
                    <td className="py-3 px-3 text-right font-black">
                      <span
                        className={`text-sm ${
                          balanceOut > 0 ? 'text-sky-600 dark:text-sky-400 font-black' : 'text-slate-400'
                        }`}
                      >
                        {balanceOut} kegs
                      </span>
                    ...
                    <td className="py-3 px-3">
                      <div className="flex items-center justify-center gap-2 max-w-xs mx-auto">
                        <input
                          ...
                          placeholder="Qty..."
                          className="w-20 px-2.5 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-slate-900 dark:text-slate-100 text-xs font-mono text-center focus:outline-none focus:border-brand-500"
                        />
                        <button
                          type="button"
                          onClick={() => handleQuickReturn(cust.id)}
                          disabled={!currentInputVal || parseInt(currentInputVal) <= 0}
                          className="px-3 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:text-slate-400 text-slate-950 font-bold text-xs flex items-center gap-1 transition-all shadow-sm"
                        >
                  ...
                  return (
                    <tr key={ret.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                      ...
                      <td className="py-2.5 px-3 text-center">
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/30">
