...
import { UserRole, SupplyModel, ProductVariety, Hub, Pump } from '../types';
...
export const SettingsScreen: React.FC = () => {
  ...
  type SettingsTab = 'company' | 'kegs' | 'pricing' | 'infrastructure' | 'thresholds' | 'system' | 'hubs';
  ...
  // These fields are seeded from `settings` once above but otherwise never
  // read it again — so an external change (a factory reset, or a real
  // Supabase sync landing a value saved from another device) never reaches
  // this screen's inputs/summaries until a full reload. Resync on every
  // real `settings` change; harmless right after this screen's own save,
  // since the store then reflects exactly what was just typed.
  React.useEffect(() => {
    ...
    setRequireClosePumpReadings(settings.require_pump_readings_to_close_shift ?? true);
  }, [settings]);
  ...
  const handleDeleteHubAction = async (hub: Hub) => {
    if (denyIfNotOwner()) return;
  ...
  const handleOpenAddPumpModal = () => {
    ...
    setPumpProductIdInput(products[0]?.id || '');
    setPumpTankIdInput(physicalTanks[0]?.id || '');
    setPumpReadingInput('0');
    setPumpHubIdInput(activeHubId === 'all' ? (hubs[0]?.id || '') : activeHubId);
  ...
  // Shared by the desktop content pane (keyed off activeDesktopTab) and the
  // mobile sheet Modal (keyed off activeMobileSheet) — both show exactly one
    ...
    <div className="space-y-6 pb-16 max-w-6xl mx-auto">
      ...
      <div className="hidden split:grid split:grid-cols-12 gap-6 items-start">
        ...
        <div className="split:col-span-8 min-w-0">
          {renderSettingsTabContent(activeDesktopTab)}
        </div>
      </div>
      ...
      {activeMobileSheet && (
        ...
        >
          <div className="pb-4">
            {renderSettingsTabContent(activeMobileSheet)}
          </div>
        </Modal>
      )}
    </div>
