    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-slate-900/80 border border-slate-800">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 placeholder-slate-400 text-xs focus:outline-none focus:border-brand-500"
          />
        </div>
      </div>
        ...
        {filteredCustomers.length === 0 ? (
          ...
          filteredCustomers.map(customer => {
            ...
            return (
              <div
                key={customer.id}
                className="rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-slate-700/80 transition-all overflow-hidden shadow-sm"
              >
                {/* Main Card Header / Summary Row */}
                <div className="p-4 sm:p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  {/* Left: Customer Info */}
                  <div className="flex items-start gap-3.5">
                    <div className="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-200 font-extrabold text-sm flex-shrink-0">
                      {customer.name.slice(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-extrabold text-sm text-white">{customer.name}</h3>
                  ...
                  <div className="flex items-center justify-between lg:justify-end gap-4 border-t lg:border-t-0 pt-3 lg:pt-0 border-slate-800">
                    ...
                    <div className="text-left lg:text-right font-mono">
                      <span className="text-[10px] text-slate-400 block uppercase">Kegs Out:</span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isOverdue && (
                        <a
                          ...
                          className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/40 text-xs font-bold transition-all"
                ...
                {isExpanded && (
                  <div className="bg-slate-950/90 border-t border-slate-800 p-4 sm:p-5 space-y-3 animate-in fade-in duration-150">
      ...
      {paymentCustomerId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden p-6 space-y-5 animate-in zoom-in-95">
            ...
            <form onSubmit={handleRecordPaymentSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300 block">Quick Amount Chips</label>
                <div className="grid grid-cols-3 gap-2">
                  <button
                    type="button"
                    onClick={() => setPaymentAmount('50000')}
                    className="py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono font-bold text-slate-200 hover:border-brand-500"
                  >
                    ₦50,000
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono font-bold text-brand-400 hover:border-brand-500"
                  >
                    Full Balance
                  </button>
                </div>
              ...
              {/* Manual Amount Input */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300 block">Settlement Amount (₦)</label>
          ...
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 space-y-4 animate-in zoom-in-95">
            ...
            <form onSubmit={handleAddCustomerSubmit} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="font-semibold text-slate-300">Customer / Business Name</label>
                <input
                  type="text"
                  value={newCustName}
              ...
              <div className="space-y-1">
                <label className="font-semibold text-slate-300">Customer Tier</label>
                <select
                  value={newCustType}
                  onChange={e => setNewCustType(e.target.value as CustomerType)}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 focus:outline-none focus:border-brand-500 capitalize"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="font-semibold text-slate-300">Credit Limit (₦)</label>
                  <input
                    type="number"
                    value={newCustLimit}
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-300">Credit Terms (Days)</label>
                  <input
                    type="number"
                    value={newCustTerms}
              ...
              <div className="space-y-1">
                <label className="font-semibold text-slate-300">Phone Number (with WhatsApp)</label>
                <input
                  type="text"
                  value={newCustPhone}
